...
11. [Transactions PostgreSQL — `Serializable` + retry borné](#11-transactions-postgresql--serializable--retry-borné)
...
## 5. Gestion de l'argent — `minor units`
...
### Pourquoi `minor` et pas `cents`

Le mot `cents` suppose implicitement 1 unité = 100 minor units (centimes).
Pour **XAF** (Franc CFA, marché cible Cameroun) c'est faux : XAF a un
**exposant 0** (1 XAF = 1 minor unit). Renommer en `minor` retire cette
fuite d'implémentation EUR et ouvre la voie aux autres devises à 0 décimales
(JPY, KRW) sans migration de schéma. Cf. ADR-0003 amendée S1-002 (Finding 5).

### Ce qu'on ne fait JAMAIS

- ❌ `Float` ou `Decimal` pour un prix (rounding cumulatif).
- ❌ `priceEuros: 19.99` — toujours `priceMinor: 1999` (ou `priceMinor: 25_000` pour XAF).
- ❌ Calcul `Math.round(value * 100)` côté UI → la conversion est faite au moment
  de la saisie, **une seule fois**, et stockée en minor units.
- ❌ Comparaison de prix entre deux devises sans conversion explicite.
- ❌ Affichage manuel du prix dans un template → on passe par
  `<Money minor={…} currency={…} />` (`src/ui/components/money.tsx`).

### Affichage

- Format français par défaut : `19,99 €` (locale `fr-FR`, currency EUR).
- Format XAF : `12 500 FCFA` (pas de décimales, code `XAF`).
- Le helper `<Money>` est l'unique point de formatage ; **aucun** `toLocaleString`
  direct dans les composants.
- L'exposant est lu depuis une **lookup locale** (constante TypeScript,
  cf. `src/lib/currency.ts`) — **pas** d'appel API pour les taux de change.
...
3. Une fois mergée, l'ADR a force de convention **jusqu'à ce qu'une nouvelle ADR la remplace**.

### ADR verrouillées en Sprint 1

- **ADR-0001** : Auth admin via cookie httpOnly + middleware (pas NextAuth).
- **ADR-0002** : `PaymentProvider` interface profonde, Stripe en premier adaptateur — **amendée** S1-002 (Finding 1 : refund async via webhook `charge.refunded`) et **ré-amendée** S1-016 (Finding D9 : `REFUND_PENDING` vit dans `PaymentStatus` uniquement, pas dans `OrderStatus`).
- **ADR-0003** : Prix en `minor units` (Int) partout, devise ISO-4217 — **amendée** S1-002 (Finding 5 : renommage `*Cents` → `*Minor`).
- **ADR-0004** : Stock décrémenté à la confirmation de paiement, pas au panier — **amendée** S1-002 (Findings 2/4/8 : TTL réservation 30 min, Order créé avec createIntent, isolation `Serializable`, check constraints `>= 0`) et **ré-amendée** S1-016 (Finding D9 : webhook répond 200 sur violation de contrainte, retry borné `40001`, job de réconciliation sort en S2, retrait `SELECT FOR UPDATE`).
- **ADR-0005** : Webhooks Stripe idempotents via table `WebhookEvent` + worker de reprise.
- **ADR-0006** *(nouvelle)* : Cartographie dépendances par carte + check constraints sur `Stock` (Findings 6, 8) — **amendée** S1-016 (Finding D9 : `CHECK` en SQL brut dans la migration, pas de `@@check` Prisma ; `REFUND_PENDING` ajouté à `PaymentStatus`).
- **ADR-0007** *(nouvelle)* : Stratégie de migration Prisma en prod via init container Coolify (Finding 7).
...
## 11. Transactions PostgreSQL — `Serializable` + retry borné
...
DAT §7). Pas de `READ COMMITTED` + `SELECT FOR UPDATE` : un seul mécanisme
...
## Annexe — Cartographie des dépendances par carte (cf. ADR-0006)
...
**Retrait validé** (vs DAT §6) : `@t3-oss/env-nextjs@0.11.1` n'est **pas**
installé. Remplacé par **zod pur** dans `src/lib/env.ts` (commentaire WAR ROOM
backend-dev, 22:24). Si quelqu'un souhaite réintroduire env-nextjs, il faut une
ADR (cf. §9).
