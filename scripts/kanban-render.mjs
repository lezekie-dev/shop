...
body{font:14px/1.5 ui-sans-serif,system-ui,sans-serif;background:${CREAM};color:${BODY};margin:0;padding:34px 30px 40px}
h1{font-family:Georgia,"Times New Roman",serif;font-size:2.1rem;letter-spacing:-.02em;color:${INK};margin:0 0 4px}
...
.card{border:1px solid ${BORDER};border-radius:9px;padding:9px 10px;margin-bottom:8px;background:${CREAM}}
...
.card__title{font-weight:600;color:${INK};margin:0 0 3px;font-size:13px}
.card__proof{color:${MUTED};font-size:11.5px;margin:0}
table{width:100%;border-collapse:collapse;background:#fff;border:1.5px solid ${BORDER};border-radius:10px;overflow:hidden;font-size:13px}
th{background:${ACCENT};color:#fffaf5;text-align:left;padding:8px 11px;font-size:11px;text-transform:uppercase;letter-spacing:.06em}
td{padding:8px 11px;border-bottom:1px solid ${BORDER}}
...
const p = await (await b.newContext({ viewport: { width: 1560, height: 1100 }, deviceScaleFactor: 2 })).newPage();
